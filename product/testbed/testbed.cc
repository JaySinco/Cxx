...
HWND createSimpleWindow(const std::string &classname, WNDPROC windowProc,
...
LRESULT CALLBACK textWindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        TextOut(hdc, RL(0.003), RL(0.003), gBoard.mousePos.c_str(), gBoard.mousePos.size());
...
void createMessageLoop()
{
    ...
    gWindow = createSimpleWindow("hintWindow", textWindowProc, "#FFFFFF", 0.8, RL(0.07), RL(0.02));
    MSG msg;
...
int main()
{
    std::thread thread(createMessageLoop);
    // Sleep(3000);
