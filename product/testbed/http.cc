...
#define MAKE_DETAIL_URL(x) \
    (std::string("https://") + x + "/cgi-bin/mmwebwx-bin")

using namespace cxx;

const std::string BASE_DOMAIN = "https://login.weixin.qq.com";
const std::string USER_AGENT = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 "
...
void WechatClient::login()
{
    ...
    wxInitUrl << logInfo.url << "/webwxinit"
              << "?r=" << (-1 * localTime / 1579)
...
void WechatClient::waitForScanQRCode()
{
    ...
    const std::string showQRUrl = "http://api.qrserver.com/v1/create-qr-code/?data=https://"
                                  "login.weixin.qq.com/l/";
    const std::string chromePath = "C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe";
    const std::regex uuid_patt(
    ...
    while (true) {
        ...
        if (!std::regex_search(uuidResp.body, uuid_match, uuid_patt) || uuid_match.size() != 3 || uuid_match[1] != "200") {
            LOG(INFO) << "failed to match window.QRLogin.uuid, response="
                      << QUOT(uuidResp.body);
            exit(-1);
        }
        ...
        CreateProcessA(NULL, (char*)cmd.c_str(), NULL, NULL, TRUE, 0, NULL, NULL,
            &si, &pi);
        bool hintConfirm = true;
        while (true) {
            std::ostringstream checkUrl;
            time_t localTime = std::time(nullptr);
            checkUrl << "/cgi-bin/mmwebwx-bin/login?loginicon=true&uuid=" << uuid
                     << "&tip=1&r=" << (-1 * localTime / 1579) << "&_=" << localTime;
            auto checkResp = httpGet(BASE_DOMAIN + checkUrl.str(), agentHeader);
            if (checkResp.code != 200) {
                LOG(INFO) << "failed to connect mmwebwx, responseCode="
                          << checkResp.code;
                exit(-1);
            }
            std::smatch code_match;
            if (!std::regex_search(checkResp.body, code_match, code_patt) || code_match.size() != 2) {
                LOG(INFO) << "failed to match window.code, response="
                          << QUOT(checkResp.body);
                exit(-1);
            }
...
void WechatClient::processLogInfo(const std::string& context)
{
    ...
    if (!std::regex_search(context, redirect_match, redirect_patt) || redirect_match.size() != 2) {
        LOG(INFO) << "failed to match window.redirect_uri, response="
                  << QUOT(context);
        exit(-1);
    }
    ...
    if (redirectResp.code != 200 && redirectResp.code != 301) {
        LOG(INFO) << "failed to connect redirect, responseCode="
                  << redirectResp.code;
        exit(-1);
    }
    ...
    if (logInfo.skey == "" || logInfo.wxsid == "" || logInfo.wxuin == "" || logInfo.passTicket == "") {
        LOG(ERROR) << "wechat account may be LIMITED to log in WEB wechat, dom="
                   << QUOT(redirectResp.body);
        exit(-1);
    }
    ...
    nlohmann::json baseq = { { "BaseRequest",
        { { "Skey", logInfo.skey },
            { "Sid", logInfo.wxsid },
            { "Uin", logInfo.wxuin },
...
HttpResponse httpMethod(CURL* curl,
...
HttpResponse WechatClient::httpGet(const std::string& url,
    const std::vector<std::string>& headers)
{
    return httpMethod(curl, "GET", url, headers, "");
}

HttpResponse WechatClient::httpPost(const std::string& url,
    const std::vector<std::string>& headers,
    const std::string& data)
{
