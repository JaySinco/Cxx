...
using bsize_t = uint8_t; // 棋盘大小内存类型
...
constexpr float BN_MVAR_INIT = 1.0f; // 批标准化初始方差
...
const std::map<int, float> LR_SETTING_MAP = {
    ...
    { 10000, BASE_LEARNING_RATE * 1e-1f },
    { 80000, BASE_LEARNING_RATE * 1e-2f },
};
constexpr int EXPLORE_STEP = 20; // 前N步探索
constexpr int EPOCH_PER_GAME = 1; // 每自我对弈一局，训练网络的次数
constexpr int TEST_PURE_ITERMAX = 1000; // 纯MCTS迭代步数
constexpr int TRAIN_DEEP_ITERMAX = 400; // 深度MCTS迭代步数
constexpr float C_PUCT = 1.0; // 探索水平
constexpr float NOISE_RATE = 0.2; // 噪音
constexpr float DIRICHLET_ALPHA = 0.3; // 狄利克雷分布
// Logging Param
constexpr int MINUTE_PER_LOG = 3; // 打印训练日志周期
constexpr int MINUTE_PER_SAVE = 30; // 保存参数周期
constexpr int MINUTE_PER_BENCHMARK = 15; // 对战纯MCTS周期
constexpr bool DEBUG_MCTS_PROB = false; // 是否打印MCTS概率
constexpr bool DEBUG_TRAIN_DATA = false; // 是否打印训练过程棋盘
